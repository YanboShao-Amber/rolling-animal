...
func _ready() -> void:
	# 加入 "resettable" 组：玩家死亡重生时 RespawnManager 会调 reset_state() 让蘑菇重新出现。
...
func _on_body_entered(body: Node2D) -> void:
	...
	# 5. 不再销毁蘑菇：保留节点（保持隐藏/无碰撞），方便玩家死亡时 reset_state() 复原重现。
...
## 关卡设定保证吃蘑菇后的 3 秒缩小期内不会死亡，所以复位时 buff 一定已结束，无需处理体型上下限。
